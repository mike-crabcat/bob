-- Rebuild notifications with expanded type support and delivery bookkeeping.
...
CREATE TABLE notifications_v2 (
    ...
    notification_type TEXT NOT NULL CHECK (
        notification_type IN ('needs_input', 'event_reminder', 'task_assignment', 'task_result', 'project_result')
    ),
...
SELECT
    ...
    COALESCE(delivery_attempt_count, 0),
    last_delivery_at,
    last_delivery_error,
    COALESCE(next_delivery_at, created_at),
    title,
    message,
    COALESCE(metadata, '{}'),
    sequence_number,
