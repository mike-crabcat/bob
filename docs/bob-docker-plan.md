...
- **Host-side companions**: `whatsappbridge.service` (Go binary from `services/whatsappbridge/bin`), `openclaw-gateway.service` (separate software, not in this repo — primary's LLM traffic routes to it on localhost). The radio (`bob-fm` unit) is host-side and calls the API on `127.0.0.1:8420`. (A `whatsapp-media-cleanup.timer` pruned `workspace/whatsapp-media` at 30 days; removed 2026-09-05 — galleries own their files under `people/`, incoming media now persists.)
