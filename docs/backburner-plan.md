...
| D6 | **DMs only in v1** (gate on `call_category == "whatsapp_incoming"` plus chat_kind check). | Holding acks in groups are noise; group member-change turns are a different category anyway. |
