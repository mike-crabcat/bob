...
| D6 | **DMs only in v1** (gate on `call_category == "whatsapp_incoming"` plus chat_kind check). *Widened to groups at deploy, 2026-08-30: Mike asked for all conversations, and the first half-hour of live traffic was 100% group turns (7 slow, worst 215s) — the DM gate left the feature idle.* | Holding acks in groups are noise; group member-change turns are a different category anyway. |
