...
### Added
- `set_mcp_server_scope` MCP admin tool (owner-only): when the operator asks in chat to make a server global, Bob can now act on it — scope='global' flips `is_global` (with alias tolerance), scope='conversation' reverts. Non-owners are hard-refused (widening a server to every conversation is not theirs to ask; no approval flow by design). Motivated by Mike's "I approve it to be global. Is that something you can act on?" DM, which Bob could only correctly deflect before

### Fixed
- Marker-swap parrot in the send-tool rescue's echo guard: GLM-5.3-flash echoed the inbound stimulus with the `[NEW — awaiting your reply]` marker swapped for its own bracketed tag (`[Proposing, as my bit…] …`), defeating both the literal-marker check and the exact-match check, and the rescue mailed Mike his own approval back (2026-09-16, Mike DM). Leading bracketed segments are now stripped before the exact comparison — tag + verbatim-echo is suppressed, tag + genuinely different reply still rescues
- MCP http-server auth (first live agent registration, 2026-09-15): Bob registered `zai-web-search` at Mike's request but put the API key in `env` (stdio-only, ignored for http) — the server got no `Authorization` header and every refresh failed on Z.ai's non-JSON-RPC `1001` error envelope, while the surfaced error read only "ExceptionGroup: unhandled errors in a TaskGroup". Three fixes: `${VAR_NAME}` references in `env_json`/`headers_json` values now expand from the service environment at connect time (registration stores the reference, never the secret — approval summaries and dashboard GETs stay clean, and Bob's instinct becomes the supported pattern; unresolvable refs surface as an actionable cache error); all MCP failure surfaces (refresh cache error, call-tool error strings, `test_mcp_server`) unwrap anyio ExceptionGroups to the root cause; the register tool's `env`/`headers` docs now say plainly which transport each applies to, with the `Bearer ${VAR}` example. The live row was repaired to `Authorization: Bearer ${BOB_ZAI_API_KEY}` and verified end-to-end (list_tools + a real web_search_prime call)

## 2026-09-14

### Added
- Agent-side MCP registration (migration `008`): Bob can register and attach MCP servers **at a trusted contact's request** — the admin tools (`register_mcp_server`/`attach_mcp_server`/`detach_mcp_servers`/`list_mcp_servers`/`test_mcp_server`) attach only in human-initiated turns from a trusted contact, so untrusted requesters never even see them. The owner registers directly; any other trusted contact's request parks an `mcp_server` approval in the owner's DM (approvals machinery, type allowlist extended via the standard rebuild), and approving executes the registration deterministically from the stored proposal, attaches the server to the requesting conversation, and wakes it with the confirmation — every non-owner registration gets the owner's eyes before any stdio command runs, which also covers prompt-injection into a trusted session. Agent-registered servers are conversation-scoped (`is_global` stays an operator decision); a `max_servers` cap (12, `BOB_MCP_MAX_SERVERS`) bounds the blast radius; `created_by` audits who asked for each server. Field validation is shared between the dashboard router and the LLM tool so the wire contract can't drift

## 2026-09-14 (earlier)
- MCP client support (migration `007`): external tool servers — stdio subprocess or streamable HTTP, via the official `mcp` SDK (2.x, lazily imported so the app and test suite run without it) — surface as native LLM tools namespaced `mcp_<server>_<tool>`. Servers are **global or attached per conversation** (`mcp_servers` + `conversation_mcp_attachments`, resolved per turn by `make_mcp_tools()` beside `build_common_tools()` at the four channel dispatch sites; wake paths and subagents deliberately excluded). Registration is Mike-only via the token-gated dashboard API (`/dashboard/api/mcp/servers`, `bob mcp` CLI): GETs redact `env`/`header` values as `"***"`, PUT round-trips a `"***"` value as keep-stored, and the agent never gets registration powers. Sessions are connection-per-call under per-server timeouts (the SDK's anyio-scoped context managers don't fit persistent FastAPI sessions; a TTL-refreshed cache serves tool listings without paying connect costs), with tool/schema caps and per-server allow/deny tool filters against prompt bloat. stdio children run outside the workspace sandbox with an allowlist-only child env — the service env (LLM keys) never leaks into spawned servers. Kill switches widest-first: per-row `enabled=0` → detach → `BOB_MCP_ENABLED=off` → drop the package. Unit + API tests run without the SDK; a real stdio round-trip integration test (`tests/test_mcp_integration.py`, local MCPServer echo fixture) skips where the SDK is absent
