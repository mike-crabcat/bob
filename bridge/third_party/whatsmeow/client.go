...
type Client struct {
	...
	paired                atomic.Bool
	// lastPairRefs caches the raw refs from the most recent pair-device IQ
	// so a companion_reg_refresh (server retiring the advertised ADV secret,
	// Sept 2026) can re-render QR codes without advancing the ref pool.
	lastPairRefs [][]byte
