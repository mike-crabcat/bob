...
func (cli *Client) handlePairDevice(ctx context.Context, node *waBinary.Node) {
	...
	for i, child := range pairDevice.GetChildren() {
		...
		cli.lastPairRefs = append(cli.lastPairRefs, content)
...
// handleCompanionRegRefresh reacts to the server retiring the advertised ADV
// secret (WhatsApp server-side change observed from late July 2026; protocol
// answer documented by WhiskeySockets/Baileys PR #2765). The QR codes being
// advertised embed the old secret, so scans fail server-side until the client
// mints a fresh 32-byte secret and re-renders the codes for the CURRENT refs
// — refs are not consumed (advancing them drains the pool), the QR timer is
// untouched, and the ack is already sent by the notification handler.
// Registered clients are skipped: rotating there would break verification of
// a completed or in-flight pairing that depends on the current secret.
func (cli *Client) handleCompanionRegRefresh(ctx context.Context, node *waBinary.Node) {
	if cli.Store.ID != nil {
		cli.Log.Debugf("companion_reg_refresh ignored: device is registered")
		return
	}
	if len(cli.lastPairRefs) == 0 {
		cli.Log.Warnf("companion_reg_refresh: no cached pair refs to re-render")
		return
	}
	cli.Store.AdvSecretKey = random.Bytes(32)
	cli.Log.Infof("companion_reg_refresh: rotated adv secret, re-rendering %d QR ref(s)", len(cli.lastPairRefs))
	evt := &events.QR{Codes: make([]string, 0, len(cli.lastPairRefs))}
	for _, ref := range cli.lastPairRefs {
		evt.Codes = append(evt.Codes, cli.makeQRData(ref, cli.getQRClientType()))
	}
	cli.dispatchEvent(evt)
}
