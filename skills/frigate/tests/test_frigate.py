...
class Harness(unittest.TestCase):
    ...
    def test_rapid_return_digest_and_no_zombie_wake(self):
        """The 2026-09-06 miss: Mike returned to the door 60s after the first
        steer; cooldown suppressed it, nothing surfaced for 20 min, then a
        stale zombie wake. Now: suppressed events record toward a digest; the
        second one triggers ONE immediate follow-up steer; suppressed events
        never fire individually later."""
        now = time.time()
        a = frigate_event("d1", camera="front_door", start=now - 40, end=now - 30)
        self.cycle([a])
        dedups = [p["dedup_key"] for p in self.outbox_pending()]
        self.assertIn("frigate:d1", dedups)  # first sighting: normal action

        # return visits inside the 10-min test cooldown
        b = frigate_event("d2", camera="front_door", start=now - 20, end=now - 10)
        self.cycle([a, b])
        dedups = [p["dedup_key"] for p in self.outbox_pending()]
        self.assertNotIn("frigate:d2", dedups)          # no individual wake
        followups = [p for p in self.outbox_pending() if "followup" in p["dedup_key"]]
        self.assertEqual(len(followups), 0)             # only 1 suppressed: below threshold

        c = frigate_event("d3", camera="front_door", start=now - 8, end=now - 2)
        self.cycle([a, b, c])
        followups = [p for p in self.outbox_pending() if "followup" in p["dedup_key"]]
        self.assertEqual(len(followups), 1)             # threshold reached: one digest
        p = followups[0]
        self.assertEqual(p["level"], "action")
        self.assertEqual(p["body"]["suppressed_count"], 2)
        self.assertIn("back at front_door", p["summary"])

        # zombie check: re-cycle long after cooldown would have cleared —
        # d2/d3 must never emit their own individual actions
        cfg = self.cfg()
        conn = fg.db(cfg)
        fg.state_set(conn, "cd:front_door", 0)
        conn.close()
        self.cycle([a, b, c])
        dedups = [x["dedup_key"] for x in self.outbox_pending()]
        self.assertNotIn("frigate:d2", dedups)
        self.assertNotIn("frigate:d3", dedups)

        # digest list cleared: a fourth suppressed event starts a new episode
        d = frigate_event("d4", camera="front_door", start=now - 5, end=now - 1)
        self.cycle([a, b, c, d])
        followups = [x for x in self.outbox_pending() if "followup" in x["dedup_key"]]
        self.assertEqual(len(followups), 1)             # no second digest from d4 alone

    def test_digest_window_ages_out(self):
        now = time.time()
        a = frigate_event("w1", camera="front_door", start=now - 40, end=now - 30)
        self.cycle([a])
        b = frigate_event("w2", camera="front_door", start=now - 20, end=now - 10)
        self.cycle([a, b])
        # age the suppression beyond the 5-min window, then suppress again
        cfg = self.cfg()
        conn = fg.db(cfg)
        supp = json.loads(fg.state_get(conn, "supp:front_door", "[]"))
        supp[0][1] = now - 400
        fg.state_set(conn, "supp:front_door", json.dumps(supp))
        conn.close()
        c = frigate_event("w3", camera="front_door", start=now - 8, end=now - 2)
        self.cycle([a, b, c])
        followups = [x for x in self.outbox_pending() if "followup" in x["dedup_key"]]
