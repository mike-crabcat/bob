...
class WakeupPumpTask:
    ...
    async def run(self, ctx: AppContext) -> None:
        ...
        # Goal loop flip-on migration (docs/goal-execution-plan.md): active
        # room goals missing their dead-man series / loop state get ensured.
        try:
            from server.services import goal_loop
            await goal_loop.reconcile(ctx)
        except Exception:
            logger.warning("goal loop reconcile failed", exc_info=True)
