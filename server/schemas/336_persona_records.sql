...
CREATE INDEX idx_persona_records_active ON persona_records(is_active);

-- Seed from persona_config values if they exist, otherwise use defaults
-- Body content only (framing headers are hardcoded in persona.py)
...
_(What do they care about? What projects are they working on? What annoys them? What makes them laugh? Build this over time.)_',
    json_object(
        'owner_name', COALESCE((SELECT value FROM persona_config WHERE key = 'owner_name'), 'Mike'),
        'model', COALESCE((SELECT value FROM persona_config WHERE key = 'model'), 'OpenAI 5.4 mini'),
        'channel', COALESCE((SELECT value FROM persona_config WHERE key = 'channel'), 'WhatsApp'),
        'host', COALESCE((SELECT value FROM persona_config WHERE key = 'host'), 'mike-workstation')
    ),
    1,
    strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
WHERE NOT EXISTS (SELECT 1 FROM persona_records);
