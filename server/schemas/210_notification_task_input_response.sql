-- Add 'task_input_response' notification type for user-provided task input.

DROP TABLE IF EXISTS notifications_v5;
CREATE TABLE notifications_v5 (
    id TEXT PRIMARY KEY,
    entity_type TEXT NOT NULL CHECK (entity_type IN ('task', 'project', 'event')),
    entity_id TEXT NOT NULL,
    notification_type TEXT NOT NULL CHECK (
        notification_type IN ('needs_input', 'event_reminder', 'task_assignment', 'task_result', 'project_result', 'task_retry', 'task_input_response')
    ),
