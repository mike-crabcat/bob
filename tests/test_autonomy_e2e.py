...
from server.models import SuccessCriterion, TaskStatus
from server.services.base import utcnow
from server.services.project_execution_service import ProjectExecutionService
from server.services.project_autonomy_service import ProjectAutonomyService
...
async def test_db():
    ...
    db = Database(db_path=db_path, schema_dir=Path(__file__).parent.parent / "server" / "schemas", pool_size=1)
    await db.connect()
