-- Add 'submission_review' to the notification_type CHECK constraint.
-- SQLite requires a table rebuild to modify CHECK constraints.

DROP TABLE IF EXISTS notifications_v7;
CREATE TABLE notifications_v7 (
    id TEXT PRIMARY KEY,
    entity_type TEXT NOT NULL CHECK (entity_type IN ('task', 'project', 'event')),
    entity_id TEXT NOT NULL,
    notification_type TEXT NOT NULL CHECK (
        notification_type IN ('needs_input', 'event_reminder', 'task_assignment', 'task_result', 'project_result', 'task_retry', 'task_input_response', 'task_tap', 'submission_review')
    ),
