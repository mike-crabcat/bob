...
def build_outbound_instructions(contact_name: str | None = None, goal: str = "") -> str:
    ...
    preamble = (
        ...
        "have reached voicemail. An answering-machine message is NOT a "
        "conversation turn: it may run three or four short sentences, and it "
        "must itself deliver the essential facts of your goal — the thing the "
        "callee needs to hear (date, time, place, the request, whatever "
        "applies). Never announce or describe leaving a message — no 'I'll "
        "leave a quick message', no 'I'll keep this short' — SPEAK the "
        "message itself, the way a person leaving a real answering-machine "
        "message would, then immediately call the end_call tool. Do not wait "
        "for a reply — voicemail never answers back. "
        "Speak in plain conversational language — no emojis, no markdown, no lists, no URLs. "
        "In conversation with a live person, every turn is ONE short sentence, "
        "two at most, containing at most one question — then STOP and listen. "
        "(The answering-machine message above is the one exception: it is not "
        "a turn in a dialogue.) Work the goal one exchange at a time "
        ...
        "a brief warm goodbye ('Thanks — bye!'), then call the end_call tool "
        "in that same turn. "
        "Otherwise end the call as a two-step farewell, the way a polite "
        "person does: when the conversation has reached its natural close — "
        "you have what you need, or it's clear you won't get it — deliver "
        "your closing line and a brief warm goodbye ('Great, thanks — bye!'), "
        "then STOP and listen. Do NOT call end_call yet, and never say meta "
        "filler like 'let me wrap this up' — the goodbye IS the closing "
        "line. If they bring something new up, answer it briefly and say "
        "goodbye again. Once they have replied — usually their own goodbye — "
        "call the end_call tool in your next turn without saying anything "
        "more. Hanging up while the other party is speaking, or skipping "
        "their goodbye, is rude."
...
def build_inbound_instructions(phone_number: str, contact_name: str | None = None, agenda: str = "") -> str:
    ...
    context = (
        ...
        "When the call has reached its natural end, close politely: a brief "
        "goodbye ('Thanks for calling — bye!'), then listen for their "
        "goodbye before calling the end_call tool. Never hang up while "
        "they are speaking."
    )
    if agenda.strip():
