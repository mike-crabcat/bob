...
from bob_server.models import DispatchStatus
...
async def test_tap_dispatch_increments_count(ctx: AppContext):
    ...
    with patch("bob_server.services.notification_service.NotificationService.create_task_tap_notification", new_callable=AsyncMock) as mock_tap:
