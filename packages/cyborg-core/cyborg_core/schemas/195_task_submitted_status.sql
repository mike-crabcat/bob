-- Add 'submitted' task status and submitted_at timestamp.
...
CREATE TABLE tasks_v2 (
    ...
    requested_by TEXT,
    plan TEXT,
    status TEXT NOT NULL CHECK (status IN ('planning', 'pending', 'active', 'paused', 'blocked', 'submitted', 'completed', 'failed')),
    blocked_reason TEXT,
...
SELECT
    ...
    priority, parent_id, current_plan_id, retry_config,
    is_recurring, recurrence_rule, next_run_at,
    created_at, updated_at, started_at, NULL, completed_at, result,
    metadata, notification_count, last_notification_at, needs_input_since, deleted_at
FROM tasks;
